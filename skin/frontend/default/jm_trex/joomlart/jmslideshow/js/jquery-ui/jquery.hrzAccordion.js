			...
			$("#"+container+"Handle"+i).bind(settings.eventTrigger,function() {			 
						...
						if( $("#"+container+"Handle"+i).attr("rel") != container+"HandleSelected"){
						  ...
						  settings.eventAction;
							...
							$('[id*='+container+'Content]').attr("class",settings.contentContainerClass);
		
			   				$("#"+container+"Handle"+i).addClass(settings.handleClassSelected);
			   				$("#"+container+"Content"+i).addClass(settings.contentContainerClassSelected);
							   

							$("."+settings.contentWrapper).css({width: finalWidth+"px" });
							//$("."+settings.handleClass).css({height: });
	...
	$.fn.extend({
		...
		if(options){
			...
			return this.each(function(a){    		
				...
				var handleMarginLeft = $("."+settings.handleClass).css("margin-left");
		
				handleWidth =  handleWidth.replace(/px/,"");
				handleMarginLeft =  handleMarginLeft.replace(/px/,"");
				...
				if(settings.fixedWidth){
					...
					if ($.browser.msie && $.browser.version < 9) {
						$('.jm-slideshow .container').css({width: "100%"});
						containerWidth =  $("."+settings.containerClass).width();
						finalWidth = containerWidth-(elementCount*handleWidth)-(handleMarginLeft)*(elementCount-1);
					}
					else {
						$('.jm-slideshow .container').css({width: "102%"});
						containerWidth =  $("."+settings.containerClass).width();
						finalWidth = containerWidth-(elementCount*handleWidth)-(handleMarginLeft)*(elementCount+3);
					}
				}
